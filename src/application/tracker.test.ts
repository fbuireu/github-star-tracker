...
describe("trackStars", () => {
	...
	describe("email", () => {
		...
		it("sends email when changes are detected", async () => {
			...
			await trackStars();
			expect(sendEmail).toHaveBeenCalledWith(expect.objectContaining({ emailConfig, htmlBody: "<p>HTML</p>" }));
		});
	...
	describe("outputs", () => {
		...
		it("sets all outputs correctly", async () => {
			...
			expect(core.setOutput).toHaveBeenCalledWith("report-html-path", "/tmp/star-tracker-report.html");
		...
		it("sets default outputs for empty repos", async () => {
			...
			expect(core.setOutput).toHaveBeenCalledWith("report-html", `<p>${t.report.noRepositories}</p>`);
			expect(core.setOutput).toHaveBeenCalledWith("should-notify", "false");
			expect(core.setOutput).toHaveBeenCalledWith("new-stargazers", "0");
			expect(core.setOutput).toHaveBeenCalledWith("total-stars", "0");
		});
		...
		it("emits a CSV header on the empty path, like every other run", async () => {
			...
			const csv = vi.mocked(core.setOutput).mock.calls.find(([key]) => key === "report-csv")?.[1] as string;
	...
	describe("per-repo, comparison and forecast charts", () => {
		...
		const forecastData: ForecastData = {
			aggregate: {
				forecasts: [{ method: ForecastMethod.LINEAR_REGRESSION, points: [{ weekOffset: 1, predicted: 110 }] }],
			},
			repos: [],
		};
		beforeEach(() => {
	...
	describe("data flow", () => {
		...
		it("passes notificationThreshold to the measurement", async () => {
			...
			await trackStars();
			expect(measureRun).toHaveBeenCalledWith(expect.objectContaining({ notificationThreshold: "auto" }));
		});
		it("passes notificationMode to the measurement", async () => {
			...
			await trackStars();
			expect(measureRun).toHaveBeenCalledWith(expect.objectContaining({ notificationMode: NotificationMode.GAINS }));
		});
