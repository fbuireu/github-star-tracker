...
The brand files, and the only folder under `src/` that holds no code. One file: `logo.svg`, the star mark
...
- **README paths are repo-relative** (`src/assets/logo.svg`). Anything rendered outside the repository needs
  ...
  case that bites, since it is a separate repo; `docs/wiki/Star-Trend-Charts.md` already uses the absolute
