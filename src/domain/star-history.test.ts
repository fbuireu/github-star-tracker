...
describe("buildStarHistory", () => {
	...
	it("holds a repo with stars but no fetched dates flat at its true total", () => {
		...
		const repoB = result.snapshots.map((snapshot) => snapshot.repos.find((repo) => repo.fullName === "user/b")?.stars);
	...
	it("anchors a partially covered repo at its real coverage instead of the 40k cap", () => {
		...
		expect(stars.filter((count) => count < 20_000).every((count) => count % 5_000 === 0)).toBe(true);
	...
	it("reconstructs a weekly-cadence full history when maxPoints is 0", () => {
		...
		const firstGap = Date.parse(result.snapshots[1].timestamp) - Date.parse(result.snapshots[0].timestamp);
