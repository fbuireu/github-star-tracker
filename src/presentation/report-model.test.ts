...
describe("buildReportModel", () => {
	...
	describe("Top Repositories", () => {
		...
		it("ranks by Star Count and drops Removed Repositories", () => {
			...
			expect(model.topRepos.map((repo) => repo.fullName)).toEqual(["user/large", "user/middling", "user/small"]);
		...
		it("cuts the set at the requested limit", () => {
			expect(modelOf({ results, config: { topRepos: 2 } }).topRepos.map((repo) => repo.fullName)).toEqual([
				"user/large",
				"user/middling",
			]);
		});
