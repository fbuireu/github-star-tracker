...
function monotonePath(points: Point[]): string {
	...
	for (let index = 1; index < count - 1; index++) {
		tangent[index] =
			slope[index - 1] * slope[index] <= 0 ? 0 : (slope[index - 1] + slope[index]) / TANGENT_AVERAGE_DIVISOR;
	}
...
function renderSvg({
	...
	const allValues = datasets.flatMap((dataset) => dataset.data.filter((value): value is number => value !== null));
...
export function renderSvgChart({ request, locale, maxPoints, range, ...style }: RenderSvgChartParams): string | null {
