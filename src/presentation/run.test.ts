...
describe("renderEmptyRun", () => {
	...
	it("emits a CSV header, so report-csv has one shape on every path", () => {
		expect(renderEmptyRun(makeConfig()).csv).toBe(generateCsvReport({ repos: [], summary: EMPTY_SUMMARY }));
	});
...
describe("both chart systems honour the options they share", () => {
	...
	it.each(SHARED_OPTIONS)("$name reaches the data-branch SVG and the email chart alike", ({ changed }) => {
		const base = outputs({});
