...
export function initializeDataBranch({ dataBranch, readOnly = false, token }: InitializeDataBranchParams): string {
	...
	const remoteArgs = (args: string[]): string[] => (token === undefined ? args : authenticatedArgs({ token, args }));
