...
describe("writeChart", () => {
	...
	it("creates charts directory and writes SVG file", () => {
		...
		expect(fs.writeFileSync).toHaveBeenCalledWith(path.join("/data", "charts", "star-history.svg"), svg);
	});
...
describe("commitAndPush", () => {
	...
	it("commits and pushes changes when there are staged changes", () => {
		...
		expect(
			ranGit("-c", `http.extraheader=AUTHORIZATION: basic ${CREDENTIAL}`, "push", "origin", "HEAD:star-tracker-data"),
		).toBe(true);
