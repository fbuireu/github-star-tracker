...
| 20 | `buildChartFiles({ config, chartHistories, forecastData, topRepoNames })` | presentation | Reads the histories resolved in step 13-14, so every chart ends on the same instant by construction |
