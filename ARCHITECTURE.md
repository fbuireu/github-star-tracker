...
Verified against the source; each is a real mismatch, not a guess.

- **`action.yml` omits several defaults.** Every overridable input spells out `(default X)` except `visibility` (`all`), `include-archived`, `include-forks`, `exclude-repos`, `only-repos`, `only-orgs`, `exclude-orgs` and `min-stars`, whose defaults live only in `src/config/defaults.ts`.
- **`formatCount` is never localized.** `src/domain/formatting.ts` hardcodes `new Intl.NumberFormat('en', { notation: 'compact' })`, so K/M suffixes stay English while `formatDate` follows `LOCALE_MAP`. `action.yml`'s `locale` input claims to localize "reports and emails" without that carve-out.
- **Mixed timezones in `formatting.ts`.** `formatDate` calls `toLocaleDateString` with no `timeZone` option (runner-local) while `buildAxisLabels` uses `getUTC*`. Tests use midday-UTC timestamps, so the discrepancy is never exercised.
- **`maxHistory: 0` keeps everything.** `addSnapshot` trims with `.slice(-maxHistory)`, and `slice(-0)` is `slice(0)`. `parseNumber` also accepts negatives, so `max-history: -1`, `min-stars: -5`, `top-repos: -1` etc. pass validation with untested downstream behaviour.
- **`compare-against: last-run` is asymmetric.** `getLastSnapshot` returns the newest snapshot even when its `timestamp` is unparseable; the `24h`/`7d`/`30d` branches filter such snapshots out first.
- **Timestamp parsing is split two ways inside `@domain`.** Only `snapshot.ts` and `velocity.ts` go through `toEpochMs` (`src/domain/time.ts`); `forecast.ts:132` (`snapshotDays`), `formatting.ts:59`/`:68` (`buildAxisLabels`) and `star-history.ts:97` call raw `Date.parse` behind their own `Number.isFinite` guards. Behaviour is equivalent today. `formatDate` (`formatting.ts:39`) has no guard at all and renders `Invalid Date`.
- **`writeHtmlReport` is a hidden write inside `setOutputs`** (`tracker.ts:290`). It targets `$RUNNER_TEMP || process.cwd()`, i.e. outside `dataDir`, so the file is written on read-only runs and on the empty-repos path (where it contains `<p>No repositories matched...</p>`) and survives `cleanup()`. `action.yml`'s `report-html-path` description does not mention that it persists.
- **Misleading skip-email log** (`tracker.ts:240-242`): "No star changes detected, skipping email" is also logged when `summary.changed` is true and only the notification threshold was unmet.
- **`sendEmail`'s `boolean` return gates the notification baseline but nothing else.** "configured but not delivered" (empty `email-to`) now correctly withholds the baseline advance, but it is still not reflected in any action output — `should-notify` reports the *decision*, not the delivery.
- **Stale chart files are never pruned.** Nothing deletes from `<dataDir>/charts/`, so a repo that drops out of `top-repos` leaves its `<owner>-<repo>.svg` on the data branch permanently.
- **Unvalidated regex and SMTP port.** `exclude-repos`/`only-orgs`/`exclude-orgs` accept `/pattern/flags` with no `RegExp` validation anywhere, so a malformed pattern throws an unhandled `SyntaxError`; `smtp-port` is `parseInt`-ed with no validation, so a non-numeric value yields `NaN`. Also note `only-repos` is exact-name-only (a `Set`), unlike its regex-capable siblings.
- **`smtp-password` is never passed to `core.setSecret`**, unlike the git push credential in `storage.ts`, which is.
- **Duplicated literals.** `star-tracker.yml` (`action.yml` vs `loader.ts:166`) and `587` (`action.yml` vs `DEFAULT_SMTP_PORT` in `@infrastructure/notification/email`) are each defined twice with no test pinning the copies together.
- **`dist/` contains files esbuild does not emit.** `esbuild.config.ts` produces only `index.js` and `index.js.map`, but `dist/` also holds `licenses.txt` and `sourcemap-register.js` from an earlier build tool; nothing in the repo regenerates them.
- **Stale test names.** `src/presentation/chart.test.ts:138` ("returns null for non-existent repository") actually asserts `"data":[0,0,0]` — `generatePerRepoChartUrl` never returns `null`; `src/infrastructure/notification/email.test.ts:131` ("logs the recipient address, not the message ID") passes even though `email.ts:94` does log the message ID.
...
- **Two comment blocks survive in the source tree**, against the stated no-comments convention: the JSDoc on `buildAxisLabels` (`src/domain/formatting.ts:51-57`) and the note above `MAX_REACHABLE_STARGAZERS` (`src/domain/constants.ts (the `MAX_REACHABLE_STARGAZERS` note)`).
