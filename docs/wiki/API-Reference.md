...
| `only-repos` | `string` | - | Comma-separated repo names to exclusively track (narrows what `only-orgs` already selected; it cannot bring back a repo `only-orgs` excluded) |
...
`new-stars`, `lost-stars` and `stars-changed` are per-run figures measured against the comparison baseline. They are not cumulative and carry no memory of whether an email was sent - with a daily cron and `compare-against: last-run` they mean "gains in the last 24 hours". `should-notify` is the cumulative one: its counter only resets when the threshold trips — and not even then if a configured email failed to send, which leaves the counter alone so the change is not lost. `notification-sent` is the delivery counterpart — `should-notify` is the decision, `notification-sent` is whether mail left the building.
