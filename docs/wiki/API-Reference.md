...
| `notification-sent` | `string` | Whether a **threshold-triggered** notification was delivered this run: `true` or `false`. It is `false` when SMTP is unconfigured, `email-to` is empty, or the send failed - and also when `send-on-no-changes` sent a courtesy email on an unchanged run, because that is not a notification. Do not read it as "an email left the runner" |
