...
The badge ([`stars-badge.svg`](https://github.com/fbuireu/github-star-tracker/blob/main/docs/examples/stars-badge.svg)) has no theming at all, in either direction: it is always the light palette. Its fixed dark label with an accent-coloured value is legible on both backgrounds, which is why it was left alone.
...
`lost-stars` counts Stars that left the Tracked Set, not Stars anyone took back. A repository that stops being observed is reported as a Removed Repository whose entire Star Count counts as lost, and deletion is only one of the ways that happens. A rename or a transfer, archiving a repository while `include-archived` is `false`, flipping one to private under `visibility: 'public'`, raising `min-stars` past a repository's count, or editing `exclude-repos` / `only-repos` all read exactly the same.

### Why

A Snapshot keys each repository by its full name, and a run sees only the list the API returns at that instant. Absence from that list is the only signal there is: `GET /user/repos` does not report what is missing, or why. A rename shows the consequence most clearly. The old full name disappears and the new one arrives as a New Repository, which carries no Delta because entering the Tracked Set is not growth, so the same Stars are subtracted under one name and added back under another - the report states `-N` lost against a net change of zero, and `new-stars` stays at `0`.

### Approach

- The Stars are attributed, not dropped into an aggregate. The repository keeps a row of its own: `status=removed` in the CSV, and an entry under *Removed Repositories* in the report and the email, naming the Star Count it was last seen with. What it loses is its place in `total-stars`, in the ranking, in the charts and in the Forecast, so one departure never bends the series.
- The state is not sticky. Nothing is written about the repository beyond the Snapshot the next run takes; if it returns to the Tracked Set it comes back as a New Repository with a zero Delta, and the Removed entry does not recur.
- **Do not alert on `lost-stars > 0` alone.** The [Examples](Examples) page shows a workflow that opens an issue on it; that step fires on an edited filter or a rename as readily as on a genuine unstar. Gate on a threshold, or compare `total-stars` across runs, if the difference matters to you.

Keying a Snapshot by the repository's numeric `id` instead of its full name would tell a rename from a deletion. It would also change the stored format for every existing data branch, for a distinction that resolves itself on the following run - see [ADR 0015](https://github.com/fbuireu/github-star-tracker/blob/main/docs/adr/0015-the-stored-history-declares-its-format-version.md) on what a format change costs. The comparison itself is `compareStars()` in `src/domain/comparison.ts`.
