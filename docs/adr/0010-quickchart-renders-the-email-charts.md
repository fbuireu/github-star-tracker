# Email charts are rendered by QuickChart, not by our own SVG renderer

Mail clients do not reliably display inline SVG, so the hand-written renderer that produces every Chart on the Data Branch cannot serve the email Report. The HTML Report instead embeds `quickchart.io` image URLs built from a Chart.js config, which the reader's mail client fetches at open time.
