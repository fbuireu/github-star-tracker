# Require a Personal Access Token rather than GITHUB_TOKEN

The tracker enumerates every Repository the authenticated account owns, but the `GITHUB_TOKEN` that GitHub Actions injects automatically is scoped to the triggering repository alone and cannot list anything beyond it. We therefore require the user to supply their own Personal Access Token, accepting the setup friction because there is no token GitHub issues automatically that can do the job.
