...
- The sharp edge is permanence. GitHub restricts the stargazer listing endpoint to repository admins and collaborators (ADR 0002), so a Repository the token cannot read is usually not a transient failure but a standing one. Its entry then freezes at its last good value indefinitely and it will never report a New Stargazer again — quietly, because the per-Run warning naming the Repository is the only signal.
