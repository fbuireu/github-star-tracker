...
A grace period — keep an entry for N Runs after it was last seen — would bound retention. It is not
...
- **Untracking a Repository no longer withdraws its published logins.** The remedies
  `docs/wiki/Known-Limitations.md` offers for the privacy exposure — keep the Data Branch in a private
  repository, or leave `track-stargazers` off — are unaffected and remain the supported ones. Removing an
