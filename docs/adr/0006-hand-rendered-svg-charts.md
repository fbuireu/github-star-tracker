...
neither can re-derive the window, colours, labels, titles or Milestones. They still differ on the points
[ADR 0010](./0010-quickchart-renders-the-email-charts.md) lists, which is the single home for that list, and
on appearance, which is what this decision is about.
...
- Charts have two independent renderers, and a change to one does not change the other. The QuickChart path is a lower-fidelity approximation of the same options through Chart.js semantics; exactly which options it drops is recorded in [ADR 0010](./0010-quickchart-renders-the-email-charts.md) and nowhere else.
