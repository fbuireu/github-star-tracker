# The notification baseline advances only when a notification was actually delivered

The Notification Threshold accumulates against a star total stored on the Data Branch, and that baseline used to move as soon as the threshold was crossed — before the email was handed to the SMTP server. A failed send was logged as a warning and the run continued, so the accumulated change had already been reset and was never retried: with a threshold of 500, one flaky SMTP night silently cost the user the next several weeks of accumulation. The run now sends the notification *before* persisting, and advances the baseline only if delivery actually happened.
...
Delivery is defined per channel, which matters for the `should-notify` output. When no SMTP transport is configured the output *is* the notification, so the baseline advances immediately; gating it on a send that never happens would leave `should-notify` stuck true forever after the first trip. Only a configured send that did not deliver withholds the advance — which covers both an SMTP rejection and the quieter case of `smtp-host` set with an empty `email-to`, where `sendEmail` returns `false` without throwing. Treating a non-null `EmailConfig` as proof that mail went out is the trap this decision exists to avoid.
